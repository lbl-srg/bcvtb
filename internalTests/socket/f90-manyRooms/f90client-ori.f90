...
!       instead of the array itself.
  ...
  ! Change all together.
  INTEGER, PARAMETER :: nDblMax = %nDblMax%      ! Maximum number of doubles
...
! Declarations for room model
  INTEGER, PARAMETER :: nRoo = nDblMax      ! Number of rooms
...
! Fill arrays for IPC
     ...
     IF (nDblRea .NE. nRoo) THEN
        write(*, *) "Expected ", nRoo, " values, but received only ", nDblRea
        write(*, *) "Exit simulation."
        write(11, *) "Expected ", nRoo, " values, but received only ", nDblRea
        write(11, *) "Exit simulation."
        EXIT simulate
     END IF
