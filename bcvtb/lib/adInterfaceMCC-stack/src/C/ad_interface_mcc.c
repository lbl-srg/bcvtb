...
JNIEXPORT jfloat JNICALL Java_adInterfaceReader_ADInterface_cbVIn (JNIEnv* env, jobject  obj, jint BoardNum, jint Chan , jint Gain, jfloat DataValue, jint Options)
...
{	
	...
	*/
	...
	if (call_cbVIn == 1) {
		...
		if (Gain == 1){
			printf ("\a\n");
			printf("Error: %s." "The Channel Gain %d is not valid. The Gain corresponds to a channel with 10 VOLTS UNIPOLAR.", ErrMessage, Gain);
		}
		else if (Gain == 2){
			printf ("\a\n");
			printf("Error: %s." "The Channel Gain %d is not valid. The Gain corresponds to a channel with 10 VOLTS BIPOLAR.", ErrMessage, Gain);
		}
		else if (Gain == 3){
			printf ("\a\n");
			printf("Error: %s." "The Channel Gain %d is not valid. The Gain corresponds to a channel with 5 VOLTS UNIPOLAR.", ErrMessage, Gain);
		}
		else if (Gain == 4){
			printf ("\a\n");
			printf("Error: %s." "The Channel Gain %d is not valid. The Gain corresponds to a channel with 5 VOLTS BIPOLAR.", ErrMessage, Gain);
		}
		else{
			printf ("\a\n");
			printf("Error: %s." "The Channel Gain %d is not valid. The Gain does not correspond to any voltage(current)and is not valid.", ErrMessage, Gain);
			exit(1);
		}
		return_value = -1.0;
	}
...
typedef int (__stdcall *importFunctionError)(int, int);
typedef int (__stdcall *importFunctionErrorMsg)(int, char*);
JNIEXPORT jfloat JNICALL Java_adInterfaceWriter_ADInterface_cbVOut (JNIEnv* env, jobject  obj, jint BoardNum, jint Chan , jint Gain, jfloat DataValue, jint Options)
