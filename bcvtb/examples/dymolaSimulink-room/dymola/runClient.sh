...
DYMPAT=`which dymola`
if [ ! -f /opt/dymola/bin/lib/libbcvtb_modelica.so ]; then
    echo "Error: Expected file /opt/dymola/bin/lib/libbcvtb_modelica.so"
    echo "       You may need to copy"
    echo "       $BCVTB_HOME/lib/modelica/libbcvtb_modelica.so"
    echo "       to /opt/dymola/bin/lib"
    echo "       Exit with error."
    exit 1
fi
if [ ! -f /opt/dymola/bin/lib/libbcvtb.so ]; then
    echo "Error: Expected file /opt/dymola/bin/lib/libbcvtb.so"
    echo "       You may need to copy"
