@echo off
rem ===================================================================
rem This version of RunEPlus.bat is configured to work with the BCVTB.
rem The file should be in the bin directory of the BCVTB.
rem There is no need to copy this file to the EnergyPlus directory.
rem
rem ===================================================================
 if Not "%1" == "" goto :NoErr
...
:                  "nolimit" if unlimited (used when calling readVarsESO)
 echo ===== %0 (Run EnergyPlus from BCVTB) %1 %2 ===== Start =====
 set program_path=
...
rem ======================================================================
rem This section does BCVTB related checks and variable settings.
rem ======================================================================
rem Check whether ENERGYPLUS_DIR is set 
echo Setting variables needed by the BCVTB.
if not defined ENERGYPLUS_DIR (
   echo Error. ENERGYPLUS_DIR environment variable is not set.
   echo        Make sure that you start EnergyPlus from the BCVTB,
   echo        and that the variable is set in bcvtb\bin\systemVariables-windows.properties.
   echo Exit with error.
   goto done
)

rem Make sure %ENERGYPLUS_DIR% is set to an existing directory
rem DO NOT remove the quotes in the echo statement, otherwise batch fails
rem if ENERGYPLUS_DIR contains "Program Files (x86)"!
if not exist "%ENERGYPLUS_DIR%" (
   echo Error. ENERGYPLUS_DIR does not point to an existing directory.
   echo        Make sure that you start EnergyPlus from the BCVTB,
   echo        and that the variable is set correctly in 
   echo        bcvtb\bin\systemVariables-windows.properties
   echo        ENERGYPLUS_DIR is "%ENERGYPLUS_DIR%"
   echo Exit with error.
   goto done
)

rem Overwrite the default settings,
rem which is needed for the BCVTB.
echo Resetting variables
if exist "%ENERGYPLUS_DIR%" set post_proc=%ENERGYPLUS_DIR%\PostProcess\
if exist "%ENERGYPLUS_DIR%" set weather_path=%ENERGYPLUS_DIR%\WeatherData\
if exist "%ENERGYPLUS_DIR%" set input_path=
if exist "%ENERGYPLUS_DIR%" set output_path=Outputs\

echo Setting path
rem Set the path to the directory with the dlls.
rem This must be a different directory than %program_path%, because
rem otherwise, the BCVTB cannot change the dll loading order.
set path=%path%;"%program_path%\Library"
rem ======================================================================
rem End of BCVTB related settings
rem ======================================================================
...
rem MWetter@lbl.gov: Added the following line to run E+ in current directory
if not exist Energy+.idd copy "%ENERGYPLUS_DIR%\Energy+.idd" "Energy+.idd"
...
COPY in.epw "%program_path%PreProcess\GrndTempCalc\in.epw"
MOVE BasementGHTIn.idf "%program_path%PreProcess\GrndTempCalc\BasementGHTIn.idf"
PUSHD "%program_path%PreProcess\GrndTempCalc\"
CALL RunBasement.bat "%input_path%%1" x STANDARDIN
POPD
COPY "%program_path%PreProcess\GrndTempCalc\EPObjects.txt" EPObjects.txt
IF EXIST EPObjects.txt COPY EPObjects.txt "%output_path%%1.bsmt"
:skipBasement
IF EXIST EPObjects.txt COPY in.idf+EPObjects.txt in.idf /B
IF EXIST EPObjects.txt COPY "%output_path%%1"+EPObjects.txt "%output_path%%1.expidf" /B
...
COPY in.epw "%program_path%PreProcess\GrndTempCalc\in.epw"
MOVE GHTIn.idf "%program_path%PreProcess\GrndTempCalc\GHTIn.idf"
PUSHD "%program_path%PreProcess\GrndTempCalc\"
CALL RunSlab.bat "%input_path%%1" x STANDARDIN
POPD
COPY "%program_path%PreProcess\GrndTempCalc\SLABSurfaceTemps.TXT" SLABSurfaceTemps.TXT
...
IF EXIST SLABSurfaceTemps.TXT COPY "%output_path%%1"+SLABSurfaceTemps.TXT "%output_path%%1.expidf" /B
...
rem MWetter@lbl.gov: Copy .end file.
 IF EXIST eplusout.end MOVE eplusout.end "%output_path%%1.end"
...
:   11.  Clean up directory.
 ...
 IF EXIST readvars.audit DEL readvars.audit
 IF EXIST sqlite.err  DEL sqlite.err
rem MWetter@lbl.gov: Delete copied Energy+.idd and Energy+.ini files
 IF EXIST Energy+.idd DEL Energy+.idd
 IF EXIST Energy+.ini DEL Energy+.ini
