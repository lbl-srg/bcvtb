...
rem ------------------------------------------------------------------
@set BCVTB_EP_VERSION=4.0
@set ENERGYPLUS_DIR=%cd%\clients\ep-%BCVTB_EP_VERSION%
@set BCVTB_DYMOLA_BIN=C:\Program Files (x86)\Dymola 7.3\bin\Dymola.exe
...
if not exist "%cd%\bin\env-var.bat" (
   echo Trying to get environment variables.
   cd bin
   java -jar "%cd%\lib\config\getEnvironmentVariables\build\jar\GetEnvironmentVariables.jar"
   echo Wrote environment variables to %cd%\bin\env-var.bat
   cd ..
)
...
echo Warning: Environment variable ANT_HOME does not point to Apache Ant.
...
echo          If Apache Ant is installed, set ANT_HOME to its directory,
echo          or adjust the file %cd%\bin\env-var.bat
...
java -jar  "%cd%\lib\config\getPath\build\jar\GetPath.jar" > setenv.temp
